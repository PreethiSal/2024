...
# Define the snooze logic
$snoozeCount = 0
$maxSnoozes = 3
$snoozeDuration = 5 # in seconds (5 minutes)
...
Function SnoozeToast {
    ...
    if ($snoozeCount -le $maxSnoozes) {
        Write-Output "Snooze clicked. Count: $snoozeCount"
        Start-Sleep -Seconds $snoozeDuration
        
        # Reshow the toast notification
        [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier($AppID).Show($ToastXml)
    } else {
        Write-Output "Max snooze limit reached. No more snoozes."
    }
}
...
$Action_Snooze = "SnoozeToast:"

If(($Show_RestartNow_Button -eq $True))
	{$Actions = @"
    <actions>
        <action activationType="protocol" arguments="$Action_Restart" content="Restart now" />
        <action activationType="protocol" arguments="$Action_Snooze" content="Snooze" />
    </actions>
"@
